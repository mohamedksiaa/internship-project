...
class ActionsTimeFlow extends CommonHookActions
{
	...
	public function addMoreMassActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		/* print_r($parameters); print_r($object); echo "action: " . $action; */
		if (in_array($parameters['currentcontext'], array('somecontext1', 'somecontext2'))) {		// do something only for the context 'somecontext1' or 'somecontext2'
			$this->resprints = '<option value="0"'.($disabled ? ' disabled="disabled"' : '').'>'.$langs->trans("TimeFlowMassAction").'</option>';
		}
	...
	public function loadDataForCustomReports($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['tabfamily'] == 'timeflow') {
			$head[$h][0] = dol_buildpath('/module/index.php', 1);
			$head[$h][1] = $langs->trans("Home");
			$head[$h][2] = 'home';
			$h++;

			$this->results['title'] = $langs->trans("TimeFlow");
			$this->results['picto'] = 'timeflow@timeflow';
		}
		...
		$arrayoftypes = array();
		//$arrayoftypes['timeflow_myobject'] = array('label' => 'MyObject', 'picto'=>'myobject@timeflow', 'ObjectClassName' => 'MyObject', 'enabled' => isModEnabled('timeflow'), 'ClassPath' => "/timeflow/class/myobject.class.php", 'langs'=>'timeflow@timeflow')
	...
	public function completeTabsHead(&$parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($parameters['mode'] == 'remove') {
			...
			if (in_array($element, ['context1', 'context2'])) {
				...
				$parameters['head'][$counter][0] = dol_buildpath('/timeflow/timeflow_tab.php', 1) . '?id=' . $id . '&amp;module='.$element;
				$parameters['head'][$counter][1] = $langs->trans('TimeFlowTab');
				if ($datacount > 0) {
					$parameters['head'][$counter][1] .= '<span class="badge marginleftonlyshort">' . $datacount . '</span>';
				}
				$parameters['head'][$counter][2] = 'timeflowemails';
				$counter++;
			}
