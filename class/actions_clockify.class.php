...
/**
 * \file    clockify/class/actions_clockify.class.php
 * \ingroup clockify
...
/**
 * Class ActionsClockify
 */
class ActionsClockify extends CommonHookActions
{
	/**
	...
	public function addMoreMassActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		/* print_r($parameters); print_r($object); echo "action: " . $action; */
		if (in_array($parameters['currentcontext'], array('somecontext1', 'somecontext2'))) {		// do something only for the context 'somecontext1' or 'somecontext2'
			$this->resprints = '<option value="0"'.($disabled ? ' disabled="disabled"' : '').'>'.$langs->trans("ClockifyMassAction").'</option>';
		}
	...
	public function addMoreActionsButtons($parameters, &$object, &$action, $hookmanager)
	{
		if ($parameters['currentcontext'] == 'projectcard') {
			global $langs;
			$this->resprints = '<a class="butAction" href="'.dol_buildpath('/clockify/ajax/timer.php', 1).'?action=start&fk_project='.$object->id.'">Démarrer le chrono</a>';
		}
		return 0;
	}
	...
	public function loadDataForCustomReports($parameters, &$action, $hookmanager)
	{
		...
		if ($parameters['tabfamily'] == 'clockify') {
			$head[$h][0] = dol_buildpath('/module/index.php', 1);
			$head[$h][1] = $langs->trans("Home");
			$head[$h][2] = 'home';
			$h++;

			$this->results['title'] = $langs->trans("Clockify");
			$this->results['picto'] = 'clockify@clockify';
		}
		...
		$arrayoftypes = array();
		//$arrayoftypes['clockify_myobject'] = array('label' => 'MyObject', 'picto'=>'myobject@clockify', 'ObjectClassName' => 'MyObject', 'enabled' => isModEnabled('clockify'), 'ClassPath' => "/clockify/class/myobject.class.php", 'langs'=>'clockify@clockify')
	...
	public function completeTabsHead(&$parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($parameters['mode'] == 'remove') {
			...
			if (in_array($element, ['context1', 'context2'])) {
				...
				$parameters['head'][$counter][0] = dol_buildpath('/clockify/clockify_tab.php', 1) . '?id=' . $id . '&amp;module='.$element;
				$parameters['head'][$counter][1] = $langs->trans('ClockifyTab');
				if ($datacount > 0) {
					$parameters['head'][$counter][1] .= '<span class="badge marginleftonlyshort">' . $datacount . '</span>';
				}
				$parameters['head'][$counter][2] = 'clockifyemails';
				$counter++;
			}
	...
	public function showLinkToObjectBlock($parameters, &$object, &$action, $hookmanager)
	{
		...
		$this->results = array('myobject@clockify' => array(
