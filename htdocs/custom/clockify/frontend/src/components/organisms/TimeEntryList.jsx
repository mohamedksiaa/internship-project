    ...
    <section className="space-y-6">
      ...
      {Object.entries(groups).map(([key, group]) => {
        ...
        return <div key={key} className="border-b-4 border-[#e3ebef] bg-white">
          ...
          <div className="hidden grid-cols-[minmax(180px,1.6fr)_minmax(110px,1fr)_minmax(100px,1fr)_85px_85px_85px_95px_115px] gap-4 border-b border-[#dce5ea] bg-white px-5 py-2 text-[11px] font-medium uppercase tracking-wide text-[#8a9aa4] lg:grid">
            <span>Description</span><span>Projet</span><span>Tâche</span><span>Début</span><span>Fin</span><span>État</span><span className="text-right">Durée</span><span className="text-right">Actions</span>
          </div>
          {group.map((entry) => <article key={entry.id} className="grid min-h-[64px] grid-cols-[minmax(160px,1fr)_auto] items-center gap-4 border-b border-[#dce5ea] px-5 py-3 last:border-b-0 lg:grid-cols-[minmax(180px,1.6fr)_minmax(110px,1fr)_minmax(100px,1fr)_85px_85px_85px_95px_115px]">
            <div className="min-w-0"><p className="truncate text-sm font-medium text-[#2c3e49]">{entry.note || 'Sans description'}</p><p className="mt-1 text-xs text-[#03a9f4] lg:hidden">{projectName(entry)}{entry.fk_task ? ` · ${taskName(entry)}` : ''}</p><p className="mt-1 text-xs text-[#71838f]">{entry.tags || 'Sans tags'}{entry.billable ? ' · Billable' : ' · Non billable'}</p></div>
