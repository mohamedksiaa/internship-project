...
const summary = {
  ...
  project_labels: { 6: 'NOUVEAU PROJET', 5: 'AAAAA', 8: 'test - 02', 11: 'backend', 12: 'hahahahha', 13: 'ACME-CORE', 14: 'PROJET-DELTA', 15: 'PROJET-EPSILON', 16: 'PROJET-ETA', 17: 'PROJET-ZETA' },
...
describe('buildStackedChartData — project x employee (both thresholds active at once)', () => {
  ...
  it('a same employee keeps the same segment slot across every bar (global, not per-bar, top-K)', () => {
    ...
    // (629890s across 3 projects: ACME-CORE, PROJET-EPSILON, PROJET-ETA), so he's
    ...
    const bacemSegment = segments.find((s) => s.label === 'Julien Petit');
    expect(bacemSegment).toBeTruthy();
    const tbUnited = rows.find((r) => r.label === 'ACME-CORE');
    const learn = rows.find((r) => r.label === 'PROJET-EPSILON');
    const training = rows.find((r) => r.label === 'PROJET-ETA');
