      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form onSubmit={(event) => submit(event, 1)} className="space-y-4">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button type="submit" disabled={saving || content.trim() === ''} className="rounded-xl bg-[#5B8FA8] px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-50">{saving ? t('daily_report.saving') : t('daily_report.save')}</button>
          </div>
        </form>
