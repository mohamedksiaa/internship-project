...
import { EXPECTED_ABSENCE_REASONS, useReasonLabel } from '../../utils/presenceLabels.js';
 ...
 * reason. Nothing else. The caller does the request and reports the outcome
 * through `saving` / `error`.
 */
export default function ExpectedAbsenceDialog({ user, initialDate, initialReason, saving = false, error = '', onSave, onClose }) {
  ...
  return (
    <div className="tw-fixed tw-inset-0 tw-z-50 tw-flex tw-items-center tw-justify-center tw-bg-black/40 tw-p-4" role="dialog" aria-modal="true" aria-labelledby="expected-absence-title">
      <form onSubmit={submit} className="tw-w-full tw-max-w-md tw-space-y-4 tw-rounded-lg tw-bg-white dark:tw-bg-slate-900 tw-p-6 tw-shadow-xl">
        ...
        <div>
          <span className="tw-mb-1 tw-block tw-text-xs tw-font-semibold tw-uppercase tw-tracking-wide tw-text-slate-500 dark:tw-text-slate-400">{t('users_report.presence.dialog.employee')}</span>
          <p className="tw-text-sm tw-font-medium tw-text-slate-900 dark:tw-text-slate-100">{user.label}</p>
        </div>

        <div>
          <label htmlFor="expected-absence-date" className="tw-mb-1 tw-block tw-text-xs tw-font-semibold tw-uppercase tw-tracking-wide tw-text-slate-500 dark:tw-text-slate-400">{t('users_report.presence.dialog.date')}</label>
        ...
        <div>
          <label htmlFor="expected-absence-reason" className="tw-mb-1 tw-block tw-text-xs tw-font-semibold tw-uppercase tw-tracking-wide tw-text-slate-500 dark:tw-text-slate-400">{t('users_report.presence.dialog.reason')}</label>
        ...
        <div className="tw-flex tw-justify-end tw-gap-2">
          ...
          <button type="submit" disabled={saving || !date} className="tw-rounded-lg tw-bg-[#5B8FA8] tw-px-4 tw-py-2 tw-text-sm tw-text-white hover:tw-bg-[#4A7690] disabled:tw-cursor-not-allowed disabled:tw-opacity-50">
