...
describe('buildChartAnalysisText', () => {
  ...
  it('reports the dominant category with its exact share and duration, then the runner-up — the exact reported scenario (Dimension=Projet, no crossing)', () => {
    // ACME-CORE dominant at 8h18, project-example runner-up, total 9h11.
    const text = buildChartAnalysisText({
      t,
      chartData: [
        { key: '1', label: 'ACME-CORE', value: 29880 }, // 8h18
    ...
    expect(text).toBe(
      'ACME-CORE représente 90% du temps total sur cette période, avec 8h18. Suivi de project-example avec 0h53.'
    );
  });
  ...
  it('when a crossing is active, names the dominant row AND its main crossing breakdown, not just a duplicate of the CSV header', () => {
    ...
    expect(text).toBe(
      'Alice représente 89% du temps total sur cette période, avec 8h00. La majorité provient de ACME-CORE (Projet), avec 7h00.'
    );
  });
});
